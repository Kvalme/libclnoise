...
float GetPerlin(float2 pos, float freq, float lac, int octave, float pers, int seed)
{
  float value = 0.0f;
  float signal = 0.0f;
...
__kernel void testKernel(__write_only __global image2d_t output)
{
int2 coord = (int2)(get_global_id(0), get_global_id(1));
int2 size = (int2)(get_global_size(0), get_global_size(1));

float2 pos = (float2)(coord.x / (float)size.x, coord.y / (float)size.y);
...
float val = GetPerlin(pos, freq, lacunarity, octaveCount, persistance, seed);

int color = (val+1.0f) * 255.0f;

write_imageui(output, coord, (uint4)(color));
}
